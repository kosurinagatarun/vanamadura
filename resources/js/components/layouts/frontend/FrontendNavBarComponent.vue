<template>
    <header
        :class="
            isSticky === true
                ? 'fixed top-0 left-0 z-30 w-full mb-5 sm:mb-8 shadow-xs bg-custom-header'
                : 'mb-5 sm:mb-8 shadow-xs bg-custom-header'
        "
    >
        <div class="container py-3.5 px-4 lg:py-0">
            <div class="flex items-center justify-between gap-5">
                    ...
                    <ul class="header-nav-list">
                        <li class="header-nav-item">
                            <router-link
                                ...
                                :class="
                                    checkIsPathAndRoutePathSame('/home')
                        ...
                        <li class="header-nav-item">
                            <button
                                type="button"
                                class="header-nav-menu down-arrow"
                            >
                                {{ $t("label.categories") }}
                            </button>
                            <div
                                ...
                                <div class="container">
                                    ...
                                    >
                                        ...
                                        <div v-for="category in categories">
                                            ...
                                            >
                                                <div
                                                    class="w-60 h-80 flex-shrink-0 pt-5 ltr:pl-5 rtl:pr-5"
                                                >
                                                    <img
                                                ...
                                                <div
                                                    class="w-full h-80 thin-scrolling pt-5 ltr:pr-5 rtl:pl-5"
                                                >
                                                    <div
                                                        class="w-full grid gap-5 grid-cols-3"
                                                    >
                                                        <div
                        ...
                        <li class="header-nav-item">
                            <router-link
                                class="header-nav-menu"
                                :class="
                                    checkIsPathAndRoutePathSame('/offers')
                                        ? 'router-link-active router-link-exact-active'
                                        : ''
                                "
                ...
                >
                    <button
                        class="lab-line-search text-lg flex-shrink-0"
                    ></button>
                    <input
                ...
                >
                    <button
                        type="button"
                        class="flex items-center gap-2 py-5 down-arrow"
                    >
                        <img
                            :src="language.image"
                            alt="language"
                    ...
                    >
                        <li
                            v-for="(LoopLanguage, index) in languages"
                            :key="index"
                            @click.prevent="
                                changeLanguage(
                                    LoopLanguage.id,
                                    LoopLanguage.code,
                                    LoopLanguage.display_mode
                                )
                            "
                        ...
                        >
                            <img
                                :src="LoopLanguage.image"
                                alt="flags"
                                class="w-4 flex-shrink-0"
                            />
                            <span
                                class="text-sm font-medium capitalize flex-auto"
                                >{{ LoopLanguage.name }}</span
                            >
                        </li>
                    </ul>
                </div>
                ...
                <div class="relative hidden lg:block group">
                    ...
                    >
                        ...
                        >
                            ...
                            <dl class="w-full">
                                <dt
                                    class="font-semibold capitalize whitespace-nowrap mb-0.5"
                                >
                                    {{ textShortener(profile.name, 20) }}
                                </dt>
                                <dd
                                    class="text-sm font-medium whitespace-nowrap text-text"
                                    v-if="profile.phone"
                                >
                        ...
                        <nav class="flex flex-col py-2">
                            <router-link
                                v-if="
                                    profile.role_id !==
                                        enums.roleEnum.CUSTOMER &&
                                    Object.keys(authDefaultPermission).length >
                                        0
                                "
                            ...
                            >
                                <i
                                    class="text-sm text-[#A0A3BD] lab-fill-key"
                                ></i>
                                <span
                                    class="text-sm font-medium capitalize whitespace-nowrap"
                                >
                            ...
                            >
                                <i
                                    class="text-sm text-[#A0A3BD] lab-fill-location"
                                ></i>
                                <span
                                    class="text-sm font-medium capitalize whitespace-nowrap"
                                >
                            ...
                            >
                                <i
                                    class="text-sm text-[#A0A3BD] lab-fill-logout"
                                ></i>
                                <span
                                    class="text-sm font-medium capitalize whitespace-nowrap"
                                >
    ...
    >
        <div class="flex items-center justify-between mb-4">
            ...
            <button type="button">
                <i
                    @click.prevent="hideTarget('search', 'search-active')"
                    class="lab-line-circle-cross text-xl text-danger"
                ></i>
            </button>
        </div>
        <div
        ...
        <div
            class="lg:hidden h-[calc(100vh_-_140px)] rounded-xl overflow-y-auto p-4 bg-gray-100"
        >
            <ul v-if="searchProductLists.length > 0" id="searchProductLists">
                <li
                    :key="searchProductList.name"
                    ...
                    v-for="searchProductList in searchProductLists"
                >
    ...
    >
        <div class="modal-dialog max-w-[360px] p-6 text-center relative">
            <button
                @click.prevent="
                    closeOrderNotificationModal('order-modal', 'modal-active')
                "
                    ...
                    if (
                        ...
                        onMessage(messaging, (payload) => {
                            ...
                            if (
                                ...
                                this.orderNotificationMessage =
                                    payload.notification.body;
                                const audio = new Audio(
                                    res.data.data.notification_audio
                                );
                                audio.play();
                            }
                        });
    ...
    methods: {
        ...
        searchElement: function () {
            if (this.searchProduct && this.searchProduct.length > 2) {
                ...
                url =
                    url +
                    appService.requestHandler({ name: this.searchProduct });
                axios
                    .get(url)
                    .then((res) => {
...
<style scoped>
.bg-custom-header {
    background-color: #228531 !important; /* Your desired header background color */
}
</style>
